...
import random, time, pygame, sys, string
    ...
    while True: # game loop
        if fallingPiece == None:
            ...
            if not isValidPosition(board, fallingPiece):
                    ...
                    if (globalScore >= highScore):
                        scoreFile = open('highScore.txt', 'r+')
                        scoreFile.truncate()
                        scoreFile.write(str(globalScore))
                        scoreFile.close()
        ...
        if time.time() - lastFallTime > fallFreq:
            ...
            if not isValidPosition(board, fallingPiece, adjY=1):
                ...
                global globalScore
                globalScore = score
                level, fallFreq = calculateLevelAndFallFreq(score, totalLines)
                fallingPiece = None
            else:
...
def calculateLevelAndFallFreq(score, lines):
    # Based on the score, return the level the player is on and
    # how many seconds pass until a falling piece falls one space.
    level = int(lines / 10) + 1
    if level <= 21:
        fallFreq = 0.45 - (level * 0.02)
    else:
        fallFreq = 0.01
...
def drawStatus(score, level):
    ...
    levelRect.topleft = (WINDOWWIDTH - 180, 50)
    ...
    if (highScore < score):
        highScore = score;
    highSurf = BASICFONT.render('High Score: %i' % highScore, True, TEXTCOLOR)
    highRect = highSurf.get_rect()
    highRect.topleft = (WINDOWWIDTH - 180, 80)
...
def drawNextPiece(piece):
    ...
    nextRect.topleft = (WINDOWWIDTH - 180, 110)
